...
1. Performs speaker diarization on participants' merged audio files. 
2. Synchronizes the result with each participant's eye tracking data.

INPUTS:
- "voicerecording_all.wav" audio file generated by merging sound files extracted 
from participants' eye tracking videos
- "aruco-dist_T.csv", "aruco-dist_A.csv", "aruco-dist_P.csv" 
csv files containing objective gaze coordinates relative to the aruco marker

OUTPUTS:
- "ETandSpeechA.csv", "ETandSpeechT.csv", and "ETandSpeechP.csv"
 merged gaze and speech data for each participant
"""
