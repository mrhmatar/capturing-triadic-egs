...
# Inspect missing and nonsensical values
#---------------------------------------

# Aruco marker detection rate
100-round(sum(dfT['aruco_IDs'].isna())*100/len(dfT), 1) #99.7%
100-round(sum(dfA['aruco_IDs'].isna())*100/len(dfA), 1) #94.9%
100-round(sum(dfP['aruco_IDs'].isna())*100/len(dfP), 1) #100.0%

# Out-of-bounds gaze coordinates (scene resolution = 1920x1080)
...
dfA.loc[(dfA['Gaze point Y'] < 0)| (dfA['Gaze point Y'] > 1080), ('obj_Gaze_Y', 'Euclidian_distances')] = np.nan
...
# Extract relevant columns and align by shortest length
# --------------------------------------------------
...
# Code gaze events (mutual gaze; joint gaze with/without speaking)
# --------------------------------------------------
...
# Joint gaze at the 3rd participant (regardless of whether they are speaking)
ATP['jointGazeP_TA'] = 0
...
# Jointly gaze at the 3rd participant when they are speaking
...
# Downsample, interpolate (continuous), and drop first NaNs
#----------------------------------------------------------
...
# Prepare DataFrames for analyses (dyadic & triadic)
# --------------------------------------------------

 # Dyadic
# (Note: x for A and P negated for left-right symmetry wrt T)
...
# Rebuild ATP (downsampled) for event summaries
# ---------------------------------------------
...
# Export matrices for SUSY / mv-SUSY 
# ----------------------------------
...
print("[OK] Exported TA.txt, TP.txt, AP.txt, TAPx.txt, TAPy.txt, TAPd.txt, TAPf.txt")
